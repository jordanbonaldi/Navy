...
** Last update Sat Feb  4 15:40:45 2017 Jordan Bonaldi
*/
...
int		attack(t_server *sv, char **map, char *pos, boolean cansend)
{
  int		i;
  int		j;
  char		swap;
  ...
  if ((pos[0] && pos[1] && pos[2]) || (pos[0] && !pos[1]) ||
    ...
    {
      (cansend) ? send_message(sv, "wrong position") : 0;
      return (1);
    }
  else if (map[i][j] == '.' || map[i][j] == 'o' || map[i][j] == 'x')
    {
      (cansend) ? send_message(sv, "missed") : 0;
      (map[i][j] != 'x') ? map[i][j] = 'o' : 0;
      return (2);
    }
  else if (map[i][j] >= '0' && map[i][j] <= '9')
    {
      (cansend) ? send_message(sv, "hit") : 0;
      map[i][j] = 'x';
      return (3);
    }
  return (0);
